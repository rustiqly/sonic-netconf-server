...
def resetSwaggerDict():
    ...
    swaggerDict["securityDefinitions"] = OrderedDict()
    swaggerDict["securityDefinitions"]["basicAuth"] = OrderedDict()
    swaggerDict["securityDefinitions"]["basicAuth"]["type"] = "basic"
    swaggerDict["security"] = []
    swaggerDict["security"].append(OrderedDict())
    swaggerDict["security"][0]["basicAuth"] = []
...
class OpenApiPlugin(plugin.PyangPlugin):
    ...
    def emit(self, ctx, modules, fd):
      ...
      for module in modules:
        ...
        moduleInfo = OrderedDict()

        moduleInfo["get"] = OrderedDict()
        moduleInfo["get"]["tags"] = [module.i_modulename]
        moduleInfo["get"]["produces"] = [
            "application/yang-data+json", "application/yang-data+xml"]
        moduleInfo["get"]["parameters"] = []
        moduleInfo["get"]["responses"] = copy.deepcopy(
            merge_two_dicts(responses, verb_responses["get"]))
        moduleInfo["get"]["operationId"] = 'get_' + \
            module.i_modulename.replace('-', '_') + "_schema"
        moduleInfo["get"]["description"] = "\'OperationId: " + moduleInfo["get"]["operationId"] + "\'"

        swaggerDict["paths"][schema_path + '/' + module.i_modulename] = moduleInfo 
      ...
      genSchemaOpenAPI(modules,ctx)

def genSchemaOpenAPI(modules, ctx):
      
      resetSwaggerDict()

      baseSchemaURL = '/schemas'

      swaggerDict["tags"] = OrderedDict()
      swaggerDict["tags"]["name"] = "schemas"
      swaggerDict["tags"]["description"] = "Operations for querying schemas on SONiC"

      # General/all schemas path
      schemas = OrderedDict()
      schemas["get"] = OrderedDict()
      schemas["get"]["tags"] = ['schemas']
      schemas["get"]["produces"] = [
          "application/yang-data+json", "application/yang-data+xml"]
      schemas["get"]["parameters"] = []
      schemas["get"]["responses"] = copy.deepcopy(
          merge_two_dicts(responses, verb_responses["get"]))
      schemas["get"]["operationId"] = 'get_all_schemas'
      schemas["get"]["description"] = 'All schemas routes'

      swaggerDict["paths"][baseSchemaURL] = schemas

      schemaFn = ctx.opts.outdir + "/schema.yaml"
      fout = open(schemaFn,'w')
      code = ordered_dump(swaggerDict, Dumper=yaml.SafeDumper)
      fout.write(code)
      fout.close()
...
def swagger_it(child, defName, pathstr, payload, metadata, verb, operId=False, xParamsList=[], jsonPayload=OrderedDict()):
    ...
    if verb not in swaggerDict["paths"][verbPathStr]:
        ...
        if verb != "delete" and verb != "get":
            swaggerDict["paths"][verbPathStr][verb]["consumes"] = ["application/yang-data+json", "application/yang-data+xml"]
        swaggerDict["paths"][verbPathStr][verb]["produces"] = ["application/yang-data+json", "application/yang-data+xml"]
